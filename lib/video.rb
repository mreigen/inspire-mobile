module Video
  ...
  def self.read_from_yaml
    ...
    yml = File.open("#{Rails.root}/config/account-settings.yml") { |ym| YAML::load(ym) }
...
=end      
    ...
    when "created_at"
      video_list.sort_by! {|h| h["created_at"]}
    when "play_counts"
        ...
        function renderWidget(holderId, data, type) {
          ...
          function handleReadyStateChange() {
            ...
            for (var i = 0; i < data.length; i++) {
              ...
              var createdAtDiv = jQuery("<div class='created-at'>" + videoData["created_at"] + "</div>");
